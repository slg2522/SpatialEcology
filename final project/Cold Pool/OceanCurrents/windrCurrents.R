...
get_current_data <- function(time_range, mean_current_data, bs_current_df) {
  
  time_range <- seq(ymd_hms(paste(2022, 8, 27, 00, 00, 00, sep = "-")),
                    ymd_hms(paste(2022, 8, 27, 00, 00, 00, sep = "-")),
                    by = "1 hours"
  )
  
  mean_current_data <- rWind::seaOscar.dl(time_range, -179, -152, 50.7, 63) %>%
    rWind::wind.mean()
  
  bs_current_df <- as.data.frame(mean_current_data)
  return(bs_current_df)
}

#call the function
bs_current_df <- get_current_data()



get_wind_points <- function() {
  # convert wind data into points
  eur_wind_pts <- bs_current_df %>%
